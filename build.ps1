...
Invoke-Expression "$CAKE_EXE `"$Script`" -target=`"$Target`" -configuration=`"$Configuration`" -verbosity=`"$Verbosity`" -script-root=`"$PSScriptRoot`" $UseMono $UseDryRun $UseExperimental"
exit $LASTEXITCODE
