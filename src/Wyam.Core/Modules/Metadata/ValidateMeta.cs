...
namespace Wyam.Core.Modules.Metadata
{
    ...
    /// arbitrary tests (delegates) to ensure validity. Metatdata can be specified as optional, in which case, typing and assertion testing
    ...
    /// the document Source and Id properties to assist in identfying invalid documents.
    /// <code>
    ...
    public class ValidateMeta<T> : IModule
    {
        ...
        private List<Assertion<T>> _assertions;

        /// <summary>
        /// Peforms validation checks on metadata.
        /// </summary>
        ...
        public ValidateMeta<T> WithAssertion(Func<T, bool> execute, string message)
        {
        ...
        public IEnumerable<IDocument> Execute(IReadOnlyList<IDocument> inputs, IExecutionContext context)
        {
            return inputs.AsParallel().Select(input =>
            {
                ...
                if (!input.Metadata.ContainsKey(_key))
                {
                    ...
                    throw GetException(input, string.Format("Meta key \"{0}\" is not found.", _key));
                }
                ...
                if (!input.MetadataAs<T>().TryGetValue(_key, out value))
                {
                    ...
                    var originalStringValue = input.String(_key);
                    throw GetException(input, string.Format("Value \"{0}\" could not be converted to type \"{1}\".", originalStringValue, typeof(T).Name));
                }
        ...
        {
            var expandedErrorMessage = String.Format(
                "{0} [Source: {1}; Id: {2}]",
                message,
                document.Source,
                document.Id);
            return new Exception(expandedErrorMessage);
