    ...
    public class Tags : Pipeline
    {
        ...
        private static ModuleList GetModules() => new ModuleList
        {
            ...
            },
            {
                ...
                {
                    new Execute(ctx =>
                        new GroupByMany(BookSiteKeys.Tags, new Documents(BookSite.RawPosts))
                            .WithComparer(ctx.Bool(BookSiteKeys.CaseInsensitiveTags) ? StringComparer.OrdinalIgnoreCase : null)),
                    new Where((doc, ctx) => !string.IsNullOrEmpty(doc.String(Keys.GroupKey))),
                    new ForEach(
                        new Execute(ctx =>
                            new Paginate(
                                ctx.Get(BookSiteKeys.TagPageSize, int.MaxValue),
                                new Documents((doc, _) => doc[Keys.GroupDocuments]))),
                        new Meta(BookSiteKeys.Posts, (doc, ctx) => doc[Keys.GroupDocuments]),
                        new Meta(BookSiteKeys.Tag, (doc, ctx) => doc.String(Keys.GroupKey)),
                        new Meta(BookSiteKeys.Title, (doc, ctx) => doc.String(Keys.GroupKey)),
